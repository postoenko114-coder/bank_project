...
import com.alex.bank.services.bankBranch.BankBranchServiceImpl;
...
public class BankBranchServiceTest {
    ...
    void deleteBankBranch_ShouldBeDeleted(){
        ...
        when(bankBranchRepository.findById(fakeBankBranch.getId())).thenReturn(Optional.of(fakeBankBranch));

        bankBranchServiceImpl.deleteBankBranch(fakeBankBranch.getId());
        verify(bankBranchRepository, times(1)).findById(fakeBankBranch.getId());
        verify(bankBranchRepository, times(1)).delete(fakeBankBranch);
    }
