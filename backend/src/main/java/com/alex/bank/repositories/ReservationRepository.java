...
public interface ReservationRepository extends JpaRepository<Reservation,Long> {
    ...
    @Query("SELECT COUNT(r) > 0 FROM Reservation r WHERE r.id = :reservationId AND r.user.id = :userId")
    boolean existsByIdAndUserId(@Param("reservationId") Long reservationId, @Param("userId") Long userId);
