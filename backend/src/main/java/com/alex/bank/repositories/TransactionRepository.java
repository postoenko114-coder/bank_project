...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
    ...
    @Query("SELECT COUNT(t) > 0 FROM Transaction t " +
            ...
            "WHERE t.id = :transactionId AND ((af.user.id = :userId) OR (at.user.id = :userId))")
    boolean existsByIdAndUserId(@Param("transactionId") Long transactionId, @Param("userId") Long userId);
